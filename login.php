...
if(isset($_POST["email"]) && isset($_POST["password"])){
	...
	if($count == 1){
			...
			if (isset($_COOKIE["product_list"])) {
				...
				for ($i=0; $i < count($product_list); $i++) { 
					...
					if(mysqli_num_rows($result) < 1){
						...
						$update_cart = "UPDATE cart SET user_id = '$_SESSION[uid]' WHERE user_id = -1";
						mysqli_query($con,$update_cart);
					}else{
						//if already that product is available into database table we will delete that record
						$delete_existing_product = "DELETE FROM cart WHERE user_id = -1 AND p_id = ".$product_list[$i];
			...
			$wishlist_sql = "UPDATE wishlist SET user_id = '$_SESSION[uid]' WHERE user_id = -1";
