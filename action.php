...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["search"])){
    ...
    if(isset($_POST["get_seleted_Category"])){
        ...
        // If a search keyword is provided, search by product title (or other relevant field)
	...
	if(isset($_POST["addToCart"])){
		...
		if($count > 0){
			...
			$sql = "SELECT id FROM cart WHERE p_id = '$p_id' AND user_id = -1";
			$query = mysqli_query($con,$sql);
	...
	if(isset($_POST["addToWishlist"])){
		...
		if($count > 0){
			...
			$sql = "SELECT id FROM wishlist WHERE p_id = '$p_id' AND user_id = -1";
			$query = mysqli_query($con,$sql);
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
...
if (isset($_POST["count_item"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	$row = mysqli_fetch_array($query);
	echo $row["count_item"];
...
if (isset($_POST["count_Wishlist_item"])) {
	...
	if (isset($_SESSION["uid"])) { 
		...
		//When user is not logged in then we will count number of item in cart by using users unique ip address
		$sql = "SELECT COUNT(*) AS count_wishlist_item FROM wishlist WHERE user_id < 0 AND p_id > 0";
	}
	$query = mysqli_query($con,$sql);
	$row = mysqli_fetch_array($query);
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
...
if (isset($_POST["wishListCommon"])) {
	...
	if (isset($_SESSION["uid"])) {
		//When user is logged in this query will execute
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id FROM products a,wishlist b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//When user is not logged in this query will execute
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id FROM products a,wishlist b WHERE a.product_id=b.p_id AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
...
if (isset($_POST["removeItemFromwishList"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "DELETE FROM wishlist WHERE p_id = '$remove_id'";
	}
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
